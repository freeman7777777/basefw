...
public class PublicController {
     ...
     * 
     * @Title: loginForm
     * @Description: 请求后台登录地址，若已登录，则直接请求 /console/index
     * @return 参数
     * @return String 返回类型
     * @throws
     */
    ...
    public String loginPost(@Valid ValidAdmin validAdmin, BindingResult bindingResult, RedirectAttributes redirectAttributes, HttpServletRequest request){
        ...
        try {
            ...
            logger.info("对管理员[" + username + "]进行登录验证..验证开始");
            currentUser.login(token);
            logger.info("对管理员[" + username + "]进行登录验证..验证通过");
        }catch(UnknownAccountException uae){
            logger.info("对管理员[" + username + "]进行登录验证..验证未通过,未知账户");
            redirectAttributes.addFlashAttribute("message", "未知账户");
        }catch(IncorrectCredentialsException ice){
            logger.info("对管理员[" + username + "]进行登录验证..验证未通过,错误的凭证");
            redirectAttributes.addFlashAttribute("message", "密码不正确");
        }catch(LockedAccountException lae){
            logger.info("对管理员[" + username + "]进行登录验证..验证未通过,账户已锁定");
            redirectAttributes.addFlashAttribute("message", "账户已锁定");
        }catch(ExcessiveAttemptsException eae){
            logger.info("对管理员[" + username + "]进行登录验证..验证未通过,错误次数过多");
            redirectAttributes.addFlashAttribute("message", "用户名或密码错误次数过多");
        }catch(AuthenticationException ae){
            //通过处理Shiro的运行时AuthenticationException就可以控制用户登录失败或密码错误时的情景
            logger.info("对管理员[" + username + "]进行登录验证..验证未通过,堆栈轨迹如下");
            ae.printStackTrace();
        ...
        if(currentUser.isAuthenticated()){
            ...
            logger.info("管理员[" + username + "]登录认证通过(这里可以进行一些认证通过后的一些系统参数初始化操作)");
            String ip = IpUtil.getIpAddr(request);
            logService.insertLoginLog(username, ip, request.getRequestURL().toString());
            return "redirect:/console/index";
        }else{
            token.clear();
            return "redirect:/console/login";
